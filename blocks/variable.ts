...
export const get_variable = new Block({
    name: "get_variable",
    template: "((name) 값)",
    func: ({name}, project, {variables}) => {
        if(!variables.value[name]){
            variables.value[name] = new Variable({name, value: 0});
        }
...
export const change_variable = new Block({
    ...
    func: ({name, value}, project, {variables}) => {
        if(!variables.value[name]){
            variables.value[name] = new Variable({name, value: 0});
        }
        variables.value[name].value += Number(value);
    }
});
...
export const value_of_index_from_list = new Block({
    ...
    func: ({name, index}, project, {variables}) => {
        if(!variables.value[name]){
            variables.value[name] = new Variable({name, value: []});
        }
        return variables.value[name].value[index - 1];
    }
});
export const add_value_to_list = new Block({
    ...
    func: ({name, value}, project, {variables}) => {
        ...
        variables.value[name].value.push(value);
    }
});
export const remove_value_from_list = new Block({
    ...
    func: ({name, index}, project, {variables}) => {
        ...
        variables.value[name].value.splice(index - 1, 1);
    }
});
export const insert_value_to_list = new Block({
    ...
    func: ({name, index, value}, project, {variables}) => {
        if(!variables.value[name]){
            variables.value[name] = new Variable({name, value: []});
        }
        variables.value[name].value.splice(index - 1, 0, value);
    }
});
export const change_value_list_index = new Block({
    ...
    func: ({name, index, value}, project, {variables}) => {
        ...
        variables.value[name].value[index - 1] = value;
    }
});
export const length_of_list = new Block({
    ...
    func: ({name}, project, {variables}) => {
        ...
        return variables.value[name].value.length;
    }
});
export const is_included_in_list = new Block({
    ...
    func: ({name, value}, project, {variables}) => {
        ...
        return variables.value[name].value.includes(value);
    }
});